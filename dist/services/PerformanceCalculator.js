...
class PerformanceCalculator {
    calculateRentPaymentScore(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (payments.length === 0)
                return 100; // No payments = perfect score
            const onTimePayments = payments.filter(p => p.status === client_1.TransactionStatus.COMPLETED &&
                !p.isDue).length;
            return Math.round((onTimePayments / payments.length) * 100);
        });
    }
    calculateTenancyStability(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return Math.round((durationScore * 0.5) + // Duration most important
                (gapScore * 0.3) + // Gaps between tenancies
                (diversityScore * 0.2) // Landlord changes
            );
        });
    }
    calculateRentToIncomeRatio(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (ratio <= 0.3)
                return 100;
            if (ratio <= 0.4)
                return 85; // Midpoint of 80-90 range
            return 40;
        });
    }
    calculateUtilityCompliance(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a, _b;
            ...
            tenancies.forEach(tenancy => {
                ...
                tenancy.bills.forEach(bill => {
                    ...
                    if (bill.transactions.length > 0) {
                        ...
                        const daysLate = Math.max(0, (payment.createdAt.getTime() - bill.dueDate.getTime()) /
                            (1000 * 60 * 60 * 24));
            ...
            if (totalUtilityBills === 0)
                return 100; // Perfect score if no utility bills
            ...
            const frequencyScore = this.calculateFrequencyAdaptation(frequencyGroups, (_b = (_a = tenancies[0]) === null || _a === void 0 ? void 0 : _a.property) === null || _b === void 0 ? void 0 : _b.priceFrequency // Use first property's frequency as reference
            );
            ...
            return Math.round((paymentCompliance * 0.4) +
                (frequencyScore * 0.3) +
                (timelinessScore * 0.2) +
                (durationScore * 0.1));
        });
    }
    calculateOverdueScore(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            var _a;
            const GRACE_PERIOD_DAYS = 14;
            const now = new Date();
            // Fetch all tenant records for this user
            const allTenancies = yield __1.prismaClient.tenants.findMany({
                where: { userId },
                include: {
                    bills: {
                        include: {
                            transactions: {
                                where: {
                                    ...
                                    reference: client_1.TransactionReference.BILL_PAYMENT
                                },
                                orderBy: { createdAt: 'desc' }
                            }
                        },
                                ...
                                where: {
                                    ...
                                    reference: {
                                        in: [
                                            ...
                                            client_1.TransactionReference.LATE_FEE
                                        ]
                                    }
                                },
                                orderBy: { createdAt: 'desc' }
            ...
            if (allTenancies.length === 0)
                return 75; // Default score if no tenant data
            let totalDebt = new library_1.Decimal(0);
            let totalObligations = new library_1.Decimal(0);
            let maxDaysLate = 0;
            ...
            for (const tenancy of allTenancies) {
                ...
                const rentAmount = ((_a = tenancy.property) === null || _a === void 0 ? void 0 : _a.price) || new library_1.Decimal(0);
                totalObligations = totalObligations.add(rentAmount);
                const rentPaid = tenancy.user.transactions.some(tx => tx.reference === client_1.TransactionReference.RENT_PAYMENT &&
                ...
                for (const bill of tenancy.bills) {
                    ...
                    const billPayments = bill.transactions.filter(tx => this.decimalToNumber(tx.amount) >= this.decimalToNumber(billAmount) * 0.95);
                ...
                for (const history of tenancy.history) {
                    const expected = history.expectedRentAmount || new library_1.Decimal(0);
                    const paid = history.amountPaid || new library_1.Decimal(0);
            ...
            const lateFees = allTenancies.reduce((count, tenancy) => {
                return count + tenancy.user.transactions.filter(tx => tx.reference === client_1.TransactionReference.LATE_FEE).length;
            }, 0);
            if (lateFees > 0) {
            ...
            // ------------------------------
            if (this.decimalToNumber(totalObligations) === 0)
                return 100; // No obligations = perfect
            ...
            // Weighted composite score
            return Math.round((completenessScore * 0.5) +
                (timelinessScore * 0.3) +
                (nonPaymentScore * 0.2));
        });
    }
    calculateCommunicationScore(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (messages.length === 0)
                return 80; // Neutral score for no communication
            ...
            const finalScore = Math.round((responseScore * 0.5) +
                (toneScore * 0.3) +
                (engagementScore * 0.2));
            return Math.max(10, Math.min(100, finalScore));
        });
    }
    calculateInspectionConsistency(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            // Get all inspections for the tenant across all properties
            const inspections = yield __1.prismaClient.inspection.findMany({
                where: { tenant: { userId } },
                include: {
                    property: {
                        include: {
            ...
            if (inspections.length === 0)
                return 100; // No inspections = perfect score
            ...
            inspections.forEach(inspection => {
                ...
                if (inspection.score >= 90) {
                    ...
                    goodInspections++;
                    if (((_a = inspection.notes) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes('reminder')) ||
                        ((_b = inspection.notes) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes('follow up'))) {
                        requiresRemediation++;
                    }
                }
                else {
                    failedInspections++;
                    if (((_c = inspection.notes) === null || _c === void 0 ? void 0 : _c.toLowerCase().includes('damage')) ||
                        ((_d = inspection.notes) === null || _d === void 0 ? void 0 : _d.toLowerCase().includes('repair'))) {
                        requiresRemediation++;
                    }
                }
            });
    ...
    calculateDepositDeductions(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            // 1. Get the initial security deposit amount from first rent payment
            const initialRentPayment = yield __1.prismaClient.transaction.findFirst({
                where: {
                    userId,
                    reference: client_1.TransactionReference.RENT_PAYMENT,
                    status: client_1.TransactionStatus.COMPLETED
            ...
            if (!initialRentPayment)
                return 100; // No deposit paid = perfect score
            ...
            const [maintenanceIssues, inspections] = yield Promise.all([
                __1.prismaClient.maintenance.findMany({
                    where: {
                        tenant: { userId },
                        OR: [
                            { status: client_1.maintenanceStatus.COMPLETED, amount: { not: null } }, // Completed with cost
                            { status: { not: client_1.maintenanceStatus.COMPLETED } } // Unresolved issues
                        ],
                ...
                __1.prismaClient.inspection.findMany({
                    where: { tenant: { userId } },
                    orderBy: { createdAt: 'desc' },
                    select: {
                        score: true,
            ...
            const maintenanceDeductions = maintenanceIssues.reduce((sum, issue) => {
                if (issue.status === client_1.maintenanceStatus.COMPLETED && issue.amount) {
            ...
            const inspectionDeductions = inspections.reduce((sum, inspection) => {
                ...
                if (inspection.score < 70) {
                    ...
                    const isMajorDamage = ((_a = inspection.notes) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes('damage')) ||
                        ((_b = inspection.notes) === null || _b === void 0 ? void 0 : _b.toLowerCase().includes('repair'));
    ...
    calculateComplaintsRecord(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            // Get all violations against this tenant
            const violations = yield __1.prismaClient.violation.findMany({
                where: {
                    tenant: { userId },
                    isDeleted: false
                },
    ...
    calculateEvictionHistory(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            // Get all eviction-related violations for the tenant
            const evictionViolations = yield __1.prismaClient.violation.findMany({
                where: {
                    tenant: { userId },
                    isDeleted: false,
                    OR: [
            ...
            if (evictionViolations.length === 0)
                return 100;
    ...
    calculatePropertyCondition(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            const [violations, inspections] = yield Promise.all([
                __1.prismaClient.violation.findMany({
                    where: {
                        tenant: { userId },
                        isDeleted: false,
                        severityLevel: { in: [client_1.SeverityLevel.MODERATE, client_1.SeverityLevel.SEVERE, client_1.SeverityLevel.CRITICAL, client_1.SeverityLevel.EXTREME] }
                    },
                    orderBy: { createdAt: 'desc' }
                }),
                __1.prismaClient.inspection.findMany({
            ...
            if (inspections.length > 0) {
                const avgInspectionScore = inspections.reduce((sum, insp) => sum + insp.score, 0) / inspections.length;
            ...
            const hasUnresolvedDamage = violations.some(v => !v.actionTaken &&
                ['SEVERE', 'CRITICAL', 'EXTREME'].includes(v.severityLevel));
    ...
    calculateAgreementCompliance(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            // Get all relevant violations and lease breaches
            const [violations, leaseBreaches] = yield Promise.all([
                __1.prismaClient.violation.findMany({
                    where: {
                        tenant: { userId },
                        isDeleted: false,
                ...
                __1.prismaClient.tenantLeaseBreach.findMany({
                    where: {
                        tenantId: userId,
            ...
            violations.forEach(violation => {
                const severity = violation.severityLevel;
                switch (severity) {
                    case 'EXTREME':
                        score -= 35;
                        break;
            ...
            if (score >= 97)
                return 100; // Exceptional compliance
            if (score >= 90)
                return 95; // Excellent compliance
            if (score >= 80)
                return 85; // Minor issues
            if (score >= 70)
                return 75; // Occasional breaches  
            if (score >= 60)
                return 65; // Some compliance issues
            if (score >= 50)
                return 55; // Frequent issues
            if (score >= 40)
                return 45; // Significant non-compliance
            if (score >= 30)
                return 35; // Serious violations
            if (score >= 20)
                return 25; // Severe non-compliance
            return 10; // Extreme violations
        });
    }
    // -------------------
    calculateOverallScore(userId) {
        return __awaiter(this, void 0, void 0, function* () {
            const [rentPaymentScore, // 20%
            ...
            const overallScore = Math.round((rentPaymentScore * 0.20) +
    ...
    // Helper methods
    applyPropertyConditionTiers(rawScore) {
        ...
        if (boundedScore >= 95)
            return 100; // Spotless
        if (boundedScore >= 85)
            return 90; // Minor wear
        if (boundedScore >= 70)
            return 80; // Some damage
        if (boundedScore >= 50)
            return 65; // Significant issues
        return boundedScore; // Poor condition
    }
    calculateResponseScore(avgHours) {
        if (avgHours === 0)
            return 80; // No responses to measure
        // Score based on response time
        if (avgHours <= 12)
            return 100; // Excellent (under 12 hours)
        if (avgHours <= 24)
            return 90; // Good (under 24 hours)
        if (avgHours <= 48)
            return 75; // Moderate (under 48 hours)
        if (avgHours <= 72)
            return 60; // Poor (under 72 hours)
        return 40; // Very poor (over 72 hours)
    }
    calculateToneScore(positive, negative, total) {
        if (total === 0)
            return 80;
        const positiveRatio = positive / total;
        const negativeRatio = negative / total;
        if (negativeRatio > 0.3)
            return 40; // Aggressive/negative tone
        if (positiveRatio > 0.5)
            return 100; // Very positive tone
        if (positiveRatio > 0.3)
            return 85; // Generally positive
        return 70; // Neutral tone
    }
    calculateEngagementScore(tenantMessages, totalMessages) {
        if (totalMessages === 0)
            return 80;
        const engagementRatio = tenantMessages / (totalMessages / 2); // Ideal is 1:1 ratio
        if (engagementRatio >= 1)
            return 100; // Very engaged
    ...
    calculateInspectionFrequency(inspections) {
    ...
    calculateFrequencyAdaptation(frequencyGroups, rentFrequency) {
        ...
        if (rentFrequency && totalBills > 0) {
            ...
            alignmentScore = Math.min(100, 80 + (preferredFrequencyCount / totalBills) * 20);
        }
    ...
    calculateDepositScore(percentage) {
        if (percentage >= 95)
            return 100;
        if (percentage >= 85)
            return 90;
...
exports.default = new PerformanceCalculator();
